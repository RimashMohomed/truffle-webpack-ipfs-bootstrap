...
window.App = {
    ...
    return instanceUsed.getUsernameByIndex.call(i).then(function(_username) {
      ...
      if(ipfsHash != 'not-available') {
        ...
        $.getJSON(url, function(userJson) {
          ...
          $('#' + userCardId).find('.card-firstname').text(userJson.firstname);
          $('#' + userCardId).find('.card-lastname').text(userJson.lastname);
          $('#' + userCardId).find('.card-age').text(userJson.age);
          $('#' + userCardId).find('.card-weight').text(userJson.weight);
          $('#' + userCardId).find('.card-height').text(userJson.height);
          $('#' + userCardId).find('.card-lat').text(userJson.lat);
          $('#' + userCardId).find('.card-long').text(userJson.long);
          $('#' + userCardId).find('.card-zoom').text(userJson.zoom);
          $('#' + userCardId).find('.card-distance').text(userJson.distance);
          ...
          <div class="col-lg-3 mt-1 mb-1" id="` + userCardId + `">
            <div class="card bg-gradient-primary text-white card-profile p-1">
              <div class="card-body">
                ...
                <h6 class="card-firstname mb-2"></h6>
                <h6 class="card-lastname mb-2"></h6>
                <h6 class="card-age mb-2"></h6>
                <h6 class="card-weight mb-2"></h6>
                <h6 class="card-height mb-2"></h6>
                <h6 class="card-lat mb-2"></h6>
                <h6 class="card-long mb-2"></h6>
                <h6 class="card-zoom mb-2"></h6>
                <h6 class="card-distance mb-2"></h6>
  ...
  createUser: function() {
    ...
    var weight = $('#sign-up-weight').val();
    var height = $('#sign-up-height').val();
    var lat = $('#sign-up-lat').val();
    var long = $('#sign-up-long').val();
    var zoom = $('#sign-up-zoom').val();
    var distance = $('#sign-up-distnace').val();

    var title = $('#sign-up-title').val();
    var intro = $('#sign-up-intro').val();
    var ipfsHash = '';

    console.log('creating user on ipfs for', username, firstname, lastname, age, weight, height, lat, long, zoom, distance);
